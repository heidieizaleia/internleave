...
$student_id = $_SESSION['user_id']; // Get logged-in ID
...
<head>
    ...
    <style>
        /* CSS STYLES */
        :root { --pastel-green-light: #f1f8f6; --pastel-green-main: #a7d7c5; --pastel-green-dark: #5c8d89; --white: #ffffff; --text-dark: #2d3436; --soft-shadow: 0 10px 30px rgba(0, 0, 0, 0.05); }
        * { box-sizing: border-box; font-family: 'Quicksand', sans-serif; transition: all 0.3s ease; }
        ...
        .profile-card { background: white; padding: 40px 30px; border-radius: 25px; text-align: center; box-shadow: var(--soft-shadow); position: sticky; top: 100px; height: fit-content; }
        ...
        .form-container { background: white; padding: 50px; border-radius: 25px; box-shadow: var(--soft-shadow); position: relative; overflow: hidden; }
        ...
        input, select, textarea { width: 100%; padding: 14px; border: 2px solid #eee; border-radius: 12px; background: #fafafa; font-size: 1rem; outline: none; }
        input:focus, select:focus, textarea:focus { border-color: var(--pastel-green-dark); background: white; box-shadow: 0 5px 15px rgba(92, 141, 137, 0.15); }
        .row { display: flex; gap: 20px; } .col { flex: 1; }

        .toggle-container { background: #f0fdf4; padding: 15px; border-radius: 12px; border: 1px solid #dcfce7; display: flex; align-items: center; gap: 15px; margin-bottom: 20px; }
        ...
        .upload-area { border: 2px dashed #ccc; border-radius: 15px; padding: 20px; text-align: center; background: #fafafa; cursor: pointer; transition: 0.3s; }
        ...
        .modal-box { background: white; padding: 40px; border-radius: 30px; width: 90%; max-width: 400px; text-align: center; box-shadow: 0 30px 60px rgba(0,0,0,0.2); animation: popIn 0.3s ease-out; }
        ...
        footer { background: white; padding: 60px 20px; text-align: center; border-top: 1px solid #eee; margin-top: 40px; }
...
<body>
            ...
            <form id="leaveForm" action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col form-group">
                        ...
                        <input type="text" value="<?php echo $student_id; ?>" readonly style="background:#eee; cursor:not-allowed;">
                    </div>
                    <div class="col form-group">
                        <label>Faculty Supervisor (Optional)</label>
                        <input type="text" name="faculty_supervisor" placeholder="Enter Name if known">
                    </div>
                </div>
                ...
                <div class="form-group"><label>Total Duration</label><input type="text" id="totalDays" name="total_days" readonly style="background: #f9f9f9; color: #5c8d89; font-weight: 700;" placeholder="0 Days"></div>
    ...
    <div class="modal-overlay" id="submitModal">
        <div class="modal-box">
            <div style="font-size: 4rem; margin-bottom: 10px;">📤</div><h2 style="margin-top:0; color:#333;">Ready to Submit?</h2><div class="modal-btn-row"><button class="btn-cancel" onclick="closeSubmitModal()">Cancel</button><button class="btn-confirm" onclick="confirmSubmit()">Confirm</button></div>
        </div>
    </div>
    <div class="modal-overlay" id="logoutModal">
        <div class="modal-box">
            <div style="font-size: 4rem; margin-bottom: 10px;">👋</div><h2 style="margin-top:0; color:#333;">Logout?</h2><div class="modal-btn-row"><button class="btn-cancel" onclick="closeLogout()">Cancel</button><button class="btn-logout" onclick="confirmLogout()">Logout</button></div>
        </div>
    </div>
