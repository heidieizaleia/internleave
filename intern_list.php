...
// --- FETCH ASSIGNED INTERNS (STUDENTS) ---
// We join students with placements to ensure we only show interns assigned to THIS supervisor
$sql_interns = "SELECT 
                    s.student_id, 
                    s.full_name, 
                    s.programme_code, 
...
if (isset($_GET['view_id'])) {
    ...
    // Fetch Leave History with Impact indicators
    $h_sql = "SELECT a.*, (SELECT COUNT(*) FROM leave_impact WHERE application_id = a.application_id) as impact_count 
...
<head>
    ...
    <script>
        ...
        // Automatically open modal if view_id is in URL (matching studentlist.php logic)
    ...
    <style>
        /* Styles kept from your original intern_list.php */
        ...
        * { box-sizing: border-box; font-family: 'Quicksand', sans-serif; transition: all 0.3s ease; }
        ...
        .intern-card { background: var(--card-bg); border-radius: 25px; padding: 30px; text-align: center; box-shadow: var(--soft-shadow); border: 1px solid var(--border-color); text-decoration: none; color: inherit; display: block; }
...
<body>
    <div class="marquee-container">
        <div class="marquee-text">
            ...
            <span>✅ Tip: Review the "Leave Impact" section before making a decision.</span>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="intern-grid">
            <?php if ($interns_res->num_rows > 0): ?>
                <?php while($intern = $interns_res->fetch_assoc()): 
                    $initials = strtoupper(substr($intern['full_name'], 0, 1) . substr(strrchr($intern['full_name'], " "), 1, 1));
                ?>
                    <a href="intern_list.php?view_id=<?php echo $intern['student_id']; ?>" class="intern-card">
                        <div class="profile-img"><?php echo $initials; ?></div>
                        <h3><?php echo $intern['full_name']; ?></h3>
                        <p><?php echo $intern['programme_code']; ?> - Sem <?php echo $intern['year_semester']; ?></p>
                        <div class="stats-row">
                            <div class="stat-item"><b><?php echo $intern['pending_count']; ?></b><span>Pending</span></div>
                            <div class="stat-item"><b><?php echo $intern['taken_count']; ?></b><span>Taken</span></div>
                        </div>
                    </a>
                <?php endwhile; ?>
            <?php else: ?>
                <p style="grid-column: 1/-1; text-align: center; color: #888; padding: 50px;">No interns currently assigned to you.</p>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="profile-modal">
            ...
            <div class="modal-grid">
                <div style="text-align: center; border-right: 1px solid var(--border-color); padding-right: 40px;">
                    <div class="profile-img" style="width:120px; height:120px; font-size:3rem; margin-bottom:15px;">
                    ...
                    <div style="text-align:left; background:var(--pastel-green-light); padding:15px; border-radius:20px; font-size:0.85rem;">
                        <p><b>Student ID:</b> <?php echo $view_student['student_id']; ?></p>
                ...
                <div>
                    ...
                    <div style="max-height: 400px; overflow-y: auto;">
                        <?php if ($view_history && $view_history->num_rows > 0): ?>
                                ...
                                <div class="leave-row">
                                    ...
                                    <div style="display:flex; align-items:center; gap:10px;">
                                        <?php if($h['impact_count'] > 0): ?>
                                            <span class="impact-tag"><?php echo $h['impact_count']; ?> Impact</span>
                                        <?php endif; ?>
                                        <span class="status-badge" style="color:<?php echo ($h['status']=='Approved') ? 'var(--pastel-green-dark)' : '#ff6b6b'; ?>">
                                            <?php echo $h['status']; ?>
                                        </span>
                                    </div>
                                </div>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <p style="color:#ccc; text-align:center; margin-top:50px;">No leave history found.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-overlay" id="logoutModal">
        <div class="modal-box" style="background:var(--card-bg); padding:40px; border-radius:30px; text-align:center; width: 350px;">
            <div style="font-size: 4rem; margin-bottom: 10px;">👋</div>
            <h2 style="margin-top:0;">End Session?</h2>
            <div style="margin-top:20px;">
                <button class="btn-no" onclick="closeLogout()">Cancel</button>
                <button class="btn-yes" style="background:#ff6b6b; color:white; padding:12px 30px; border:none; border-radius:10px; font-weight:700; cursor:pointer; margin-left:10px;" onclick="confirmLogout()">Logout</button>
            </div>
        </div>
    </div>
