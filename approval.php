...
if (isset($_GET['action']) && isset($_GET['app_id'])) {
    ...
    if ($conn->query($update_sql)) {
        // Log the decision in leave_approval table
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: 'Quicksand', sans-serif; transition: all 0.3s ease; }
        ...
        .approval-card { 
            ...
            box-shadow: var(--soft-shadow); border: 1px solid var(--border-color);
            display: grid; grid-template-columns: 80px 1fr 180px; gap: 20px; align-items: center;
        }

        .student-avatar {
            width: 70px; height: 70px; background: var(--pastel-green-light); border-radius: 15px;
...
<body>
    ...
    <div class="marquee-container">
        <div class="marquee-text">
            <span>📢 Attention Supervisor: Review documents thoroughly before making a decision.</span>
            <span>✅ Documents can be clicked to open in a new browser window.</span>
        </div>
    </div>
                ...
                <?php while($row = $pending_res->fetch_assoc()): ?>
                    <div class="approval-card">
                        ...
                        <div class="details-box">
                            ...
                            <div class="meta">
                                <span><i class="far fa-calendar"></i> <?php echo $row['total_days']; ?> Day(s)</span>
                                <span><i class="fas fa-tag"></i> <?php echo $row['leave_type']; ?></span>
                            </div>
                ...
                <div class="approval-card" style="border-left: 5px solid <?php echo ($row['status'] == 'Approved') ? 'var(--success)' : 'var(--danger)'; ?>;">
                    ...
                    <div class="details-box">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <h3><?php echo $row['full_name']; ?></h3>
                            <span class="status-pill status-<?php echo $row['status']; ?>"><?php echo $row['status']; ?></span>
                        </div>
                        <div class="meta">
                            <span><i class="far fa-calendar"></i> <?php echo $row['total_days']; ?> Day(s)</span>
                        </div>
            ...
            <div id="docSection">
                ...
                <a href="#" id="modalDocLink" target="_blank" class="doc-preview">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.8rem; color:#888;">Click to open file</div>
                    </div>
                </a>
            </div>
            <button class="btn-no" onclick="closeDetailModal()" style="width:100%; margin-top:10px; border:none; padding:12px; border-radius:10px; cursor:pointer;">Close Preview</button>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="logoutModal">
        <div class="modal-box">
            ...
            <div style="margin-top:20px;">
                <button class="btn-no" onclick="closeLogout()" style="border:none; padding:12px 30px; border-radius:10px; cursor:pointer;">Cancel</button>
                <button onclick="window.location.href='logout.php'" style="background:#ff6b6b; color:white; border:none; padding:12px 30px; border-radius:10px; font-weight:700; cursor:pointer;">Logout</button>
            </div>
        </div>
    </div>
    ...
    <script>
        function openDetailModal(name, reason, docPath) {
            ...
            if (docPath && docPath !== 'none' && docPath !== '') {
                ...
                docLink.href = docPath; // The actual path from DB
                docNameText.innerText = docPath.split('/').pop(); // Show just the filename
            } else {
        ...
        window.onclick = function(e) { if(e.target.className === 'modal-overlay') { closeDetailModal(); closeLogout(); } }
    </script>
</body>
</html>